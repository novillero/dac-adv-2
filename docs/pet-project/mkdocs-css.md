...
!!! info "Важно!"
    ...
    Также вы можете использовать только одну цветовую схему. Для этого в `mkdocs.yml` оставьте настройки только одной цветовой схемы.
...
### О совместимости цветовых схем и плагина mkdocs-puml

Плагин для работы PlantUML-диаграммами [mkdocs-puml](./plant-uml.md) по умолчанию генерирует два варианта диаграммы: для светлой схемы `default` и для тёмной темы `slate`.

Если вы замените какую-то из стандартных цветовых схем авторской схемой, в этой схеме плагин будет отображать сразу два варианта диаграммы.

Если вы оставите только одну световую схему в проекте, плагин также будет отображать два варианта диаграммы.

#### Решение проблемы

Во избежание некорректной работы плагина при двух цветовых схемах используйте стандартные схемы `default` и `slate` (вы можете изменить их параметры в `extra.css`).

При использовании одной цветовой схемы в проекте добавьте в настройки плагина параметр `theme:enabled`:
...
При значении параметра `false` плагин будет генерировать только одну версию диаграммы.
